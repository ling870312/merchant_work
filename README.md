...
- **预集成 iOS 工程（含 SDK / MobileVLCKit / 插件 / 渠道 appid 与 appkey）托管在私有资产仓**。`Prepare iOS Pre-integrated Project` 用 `ASSET_PAT` 从私有仓取 SDK/插件、**只把产物回传私有仓**（绝不发布到本公开仓）；`Build iOS IPA` 仅用 `ASSET_PAT` 只读拉取预集成工程；
...
| 工作流 | Runner | 用途 |
|---|---|---|
| `prepare-ios-project.yml` | macos-14 | 一次性生成预集成 iOS 工程（瘦身 SDK 4.87 + DoorMaster-Monitor-Player + MobileVLCKit + 渠道身份），产出回传**私有资产仓** `ios-project-v1` |
...
> 以上工作流均为通用编排，不携带任何明文凭据与真实渠道身份；预集成工程（`ios-project.zip`）始终只落到**私有资产仓**，在 SDK / 原生插件变更时触发 prepare 即可。
...
```
.github/workflows/
  prepare-ios-project.yml    # 生成预集成工程（产物回传私有资产仓）
...
scripts/ci/
  ...
  integrate-plugins.rb       # 集成原生插件进 xcodeproj（prepare 用）
  inject-ios-feature-plist.py# 注册 feature.plist 插件条目（prepare 用）
  clean-privacy-strings.py   # 清理 Info.plist 隐私键（prepare 用）
...
1. **配置 Secrets**：见 [SECRETS.md](./SECRETS.md)，按清单逐个配置（含 `ASSET_PAT` 访问私有资产仓）。
2. **（SDK/插件变更时）生成预集成工程**：触发 `Prepare iOS Pre-integrated Project`，产出 `ios-project-v1` **回传私有资产仓**。
3. **打包 IPA**：触发 `Build iOS IPA`，选择渠道/版本/Gitee 分支（留空则用默认 Secret）。预集成工程自动从私有资产仓拉取最新 `ios-project-v1`。
...
- `ASSET_PAT` 授权私有资产仓：prepare 需 Contents **Read+Write**（回传预集成工程），build 仅需 Read（下载）。**不要**把 SDK / 预集成工程等资产上传到本公开仓；
